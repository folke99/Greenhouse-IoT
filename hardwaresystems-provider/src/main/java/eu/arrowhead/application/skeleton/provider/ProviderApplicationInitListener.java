...
import ai.aitia.greenhouse.hardwaresystems.HardwaresystemConstants;
...
import static ai.aitia.greenhouse.hardwaresystems.HardwaresystemConstants.*;
...
public class ProviderApplicationInitListener extends ApplicationInitListener {
	...
	protected void customInit(final ContextRefreshedEvent event) {
		...
		registerService(GET_WATERING_SERVICE_ON_DEFINITION, WATERING_SYSTEM_ON_URI, HttpMethod.POST);

		registerService(GET_WATERING_SERVICE_OFF_DEFINITION, WATERING_SYSTEM_OFF_URI, HttpMethod.POST);

		registerService(GET_LIGHTING_SERVICE_DEFINITION, LIGHTING_SYSTEM_URI, HttpMethod.POST);
	...
	private void registerService(final String serviceDefinition, final String serviceUri, final HttpMethod httpMethod) {
		...
		if (sslEnabled && tokenSecurityFilterEnabled) {
			...
			serviceRegistryRequest.setInterfaces(List.of(HardwaresystemConstants.INTERFACE_SECURE));
		} else if (sslEnabled) {
			systemRequest.setAuthenticationInfo(Base64.getEncoder().encodeToString(arrowheadService.getMyPublicKey().getEncoded()));
			...
			serviceRegistryRequest.setInterfaces(List.of(HardwaresystemConstants.INTERFACE_INSECURE));
		}
		serviceRegistryRequest.setProviderSystem(systemRequest);
		serviceRegistryRequest.setServiceUri(serviceUri);
		serviceRegistryRequest.setMetadata(new HashMap<>());
		serviceRegistryRequest.getMetadata().put(HardwaresystemConstants.HTTP_METHOD, httpMethod.name());
